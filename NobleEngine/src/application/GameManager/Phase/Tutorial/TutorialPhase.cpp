...
{
    // レンダーターゲット
    renderTargetID_ = Game::Asset::RenderTexture::CreateRenderTexture(Game::Window::GetWidth(), Game::Window::GetHeight(), "Tutorial");
...
void TutorialPhase::Initialize()
{
    nextPhase_ = Phase::Phase_None;
    context_->renderTargetIDs[static_cast<size_t>(Phase::Phase_Tutorial)] = renderTargetID_;
...
void TutorialPhase::CheckColliders()
{
...
bool TutorialPhase::LoadObstacleData(int32_t stage)
{
...
void TutorialPhase::SaveObstacleData(int32_t stage)
{
    std::string path = "assets/application/json/StageData/Obstacles.json";
    std::string key = "/Stage" + std::to_string(stage) + "/Count";
...
bool TutorialPhase::LoadLightData()
{
...
void TutorialPhase::SaveLightData()
{
    std::string path = "assets/application/json/StageData/Lights.json";
    std::string key = "/Count";
...
void TutorialPhase::ChangeCameraPhase(CameraPhase phase)
{
    switch (phase)
    {
...
void TutorialPhase::UpdateCameraPhase()
{
    switch (cameraPhase_)
    {
    case CameraPhase::Free:
